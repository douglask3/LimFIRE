...
switchFilename <- function(y, from = "/prj/ukesm/doukel/", to = 'D:/Laurens22122018/Documents/work/') {
    fname = filename(y)
    if (!grepl(from, fname)) return(y)
    fnameX = strsplit(fname, from)[[1]][2]
    fnameX = paste0(to, fnameX)
    y = brick(fnameX)[[1]]
    return(y)
}    
...
addboxPlot <- function(x, y, col, mask) {
	...
	mask[is.na(mask)] = 0.0
    #y = y[[1:5]]
    y = layer.apply(y, switchFilename)
...
addLegend <- function() {
	...
	addLegItem <- function(x, col) {
		addBoxLeg <- function(dif) {
			boxplot(c(-0.4, 0.4)+ dif, at = x, add = TRUE, border = make.transparent('black', 1.0), col = make.transparent(col, 0.97), axes = FALSE, cex = 0.2, boxwex = 0.5)
			
			lines(c(x, x), c(-0.9 - dif, 0.9 + dif), lwd = 0.5 + (0.2 + dif) * 2.5 * 1.5, col = make.transparent('black', 0.98))
			lines(x + c(-0.1, 0.1), c(0.0,0.0)+ dif * 0.25, col = make.transparent('black', 0.9))
		}
	...
	addAxisLab <- function(y, lab) {
		...
		axis(4, at = y, labels = lab, tick = FALSE, cex.axis = 0.8, las = 1, col = "#FFFFFF00", line = -0.9)
	}
	addAxisLab(-0.9, '10%')
	addAxisLab(-0.4, '25%')
...
plotAllTheBoxesAndWhiskers <- function(fname = '', ylims = c(-.6, 0.85), ytextPos = ylims[1] * 8) {
	figName = paste(figName, fname, ylog, '.png', sep ='-')
	png(figName, height = 7 * 7.2/8.5, width = 7.2, res = 300, unit = 'in')
		
        layout(rbind(c(1, 1, 1), c(1, 2,1)), heights = c(0.78, 0.22), widths = c(0.73, 0.235, 0.035))		
		par(mar = c(0, 4, 0, 1))
