...
\begin{frame}<2-5>[label=framework]
	...
	\only<7> {
		\inputMap{4.7cm}{1.33cm}{inputs_mean-popdens}{HYDE Popdens}
		\inputMap{8.33cm}{6cm}{inputs_mean-crop}{HYDE crop}
	}
    ...
    \only<5> {
    	\begin{backgroundblock}{4.2cm}{1.33cm}
    		\begin{tikzpicture}
		        ...
		        \node[align=center, font = {\small}] at (2.95cm, 4.2cm) {Global Fire Emission Database 4s (GFED)};
	        \end{tikzpicture}
	    \end{backgroundblock}
        %?% Make bigger
    }
...
\def \controlsSide#1 {
    \begin{textblock*}{11cm}(0cm,1.5cm)
        \begin{tikzpicture}
            ...
            {\draw[white, fill = white] (6.5,3.0) -- (13.0,3.0) -- (13.0,7) -- (6.5,7) -- (6.5,3.0);}
            \visible<-1> {\draw[white, fill = white] (0.0,0.0) -- ( 6.5,0.0) -- ( 6.5,3.5) -- (0.0,3.5) -- (0.0,0.0);}
            {\draw[white, fill = white] (6.5,3.0) -- (13.0,3.0) -- (13.0,0.0) -- (6.5,0.0) -- (6.5,3.0);}
...
\begin{frame}<1-3>[label=controlMapsNoLand]
	...
	\end{tikzpicture}
	\end{textblock*}
	\begin{textblock*}{14cm}(6.7cm,1.5cm)
		\only<3->{
		\includegraphics[width=5cm]{images/caseStudy/seasonal_casestudyAfrica}}
		\only<4->{
			\includegraphics[width=5cm]{images/caseStudy/seasonal_casestudyAsia1}}
	\end{textblock*}
\end{frame}
