...
mod = cbind(mod, background = 1)

plotFireVsVar <- function(i, fun, ps, 
                          col = 'green',
                          xlab = '', xlim = c(0, 2000)) {
    
    params = param(ps, return_all)
    #dev.new()
...
plotFireVsVar(c('background', 'Lightn', 'pas', 'popdens'), LimFIRE.ignitions, c('L', 'P','D', 'ig0', 'kig'), 'red', 'igntions (/m2)', c(0, 300))
