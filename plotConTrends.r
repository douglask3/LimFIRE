...
c(trend12, trend12F, trend12FF, trendFS) := findParameterTrends(NULL, 1)


#ensamble = lapply(seq(0, 1, length.out = niterations), findParameterTrends, 4)
...
plotHotspots <- function(trends, figName, limits = dfire_lims, fire_limits = limits, 
	...
	png(figName, height = 10, width = 8.5, units = 'in', res = 300)
		...
		findClustRange <- function(cl) {
			...
			index = vclusters == cl
			if (sum(index)) val = FUN(cv[index, ])
				else apply(cv[index, ], 2, FUN)
